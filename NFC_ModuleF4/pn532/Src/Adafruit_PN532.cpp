...
void DBG::println (const uint8_t *str, const uint8_t type)
{}
void DBG::println (const uint8_t *str)
{}
...
Adafruit_PN532::Adafruit_PN532(uint16_t clk, uint16_t miso, uint16_t mosi,
 ...
 _irq = -1 ,_reset = -1;
  ...
  SPI_SCT.Instance = SPI1;
  SPI_SCT.Init.Mode = SPI_MODE_MASTER;
  SPI_SCT.Init.Direction = SPI_DIRECTION_2LINES;
  SPI_SCT.Init.DataSize = SPI_DATASIZE_8BIT;
  SPI_SCT.Init.CLKPolarity = SPI_POLARITY_LOW;
  SPI_SCT.Init.CLKPhase = SPI_PHASE_1EDGE;
  SPI_SCT.Init.NSS = SPI_NSS_SOFT;
  SPI_SCT.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_256;
  SPI_SCT.Init.FirstBit = SPI_FIRSTBIT_MSB;
  SPI_SCT.Init.TIMode = SPI_TIMODE_DISABLE;
  SPI_SCT.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;
  SPI_SCT.Init.CRCPolynomial = 10;
  if (HAL_SPI_Init(&SPI_SCT) != HAL_OK)
  {
    Error_Handler();
...
//		uint8_t a = data[szPos] & 0xff;
//    PN532DEBUGPRINT.print((const uint)a , HEX);
...
bool Adafruit_PN532::isready() {
    ...
    return reply == PN532_SPI_READY;
	  ...
	  uint8_t reply;
		CS_LOW;
		HAL_SPI_Transmit(&SPI_SCT, &cmd, 1,100);
		HAL_SPI_Receive(&SPI_SCT, &reply, 1,100);
	  CS_HIGH;	
	return reply == PN532_SPI_READY;
		#endif
  #else 
...
void Adafruit_PN532::readdata(uint8_t *buff, uint8_t n) {
		...
		uint8_t cmd = PN532_SPI_DATAREAD;
		HAL_SPI_Transmit(&SPI_SCT, &cmd, 1,100);
		HAL_SPI_Receive(&SPI_SCT, buff, n,100);
	 #endif
		#ifdef PN532DEBUG
...
void Adafruit_PN532::writecommand(uint8_t *cmd, uint8_t cmdlen) {
    ...
    cmdlen++;
		packet[i++] = PN532_SPI_DATAWRITE; 		  //packet[0] = PN532_SPI_DATAWRITE;
    packet[i++] = PN532_PREAMBLE; 				  //packet[1] = PN532_PREAMBLE;
    packet[i++] = PN532_STARTCODE1;			    //packet[2] = PN532_STARTCODE1;
    packet[i++] = PN532_STARTCODE2;			  	//packet[3] = PN532_STARTCODE2;
    checksum = PN532_PREAMBLE + PN532_STARTCODE1 + PN532_STARTCODE2;
	
    packet[i++] = cmdlen;	   										//packet[4] = cmdlen;
    packet[i++] = ~cmdlen + 1;									//packet[5] = ~cmdlen + 1;
    packet[i++] = PN532_HOSTTOPN532;						//packet[6] = PN532_HOSTTOPN532;						
    ...
    spi_dev->write(packet, 8 + cmdlen);
		...
		CS_LOW;
		HAL_SPI_Transmit(&SPI_SCT, packet, i , 100);
		CS_HIGH;
